...
def plot_v(): # 训练过程中画输出电压的图,粗画看个趋势即可
    ...
    plt.figure()
    plt.plot(X, total_v)
    plt.title('R change--saved model' )
...
action_list = np.linspace(0.22, 0.50, A_DIM)
# 右边两个没用到过
...
trainer.load_models(episode=3, env=ENV, lr=LR, gamma=GAMMA, batch_size=BATCH_SIZE, date=DATE)
