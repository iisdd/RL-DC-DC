...
register(
    id=ENV,
    entry_point='env.datacenter_env:DataCenterenv',  # env文件夹里的一个叫BuckEnv的类(在buck_env.py里)
    kwargs=config
)
...
GAMMA = 0.9
REPLACE = 100
DATE = '3-19' # 日期在这改,保存模型用的
################################## 超参 #####################################
S_DIM = env.observation_space.shape[0]
A_DIM = env.action_space.n

action_list = np.linspace(-2, 2, num=9, endpoint=True)
...
total_reward = []
best_r = 80000
last_action = 279.15          # 冷冻水出水温度初始为6℃
